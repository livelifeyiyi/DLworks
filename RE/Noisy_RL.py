...
class NoisyModel(nn.Module):
	def __init__(self, dim, noisy_count):
	...
	def forward(self, encoder_output, entity_output, noisy_vec_mean, memory, training):
		# if torch.cuda.is_available():
		...
		seq_vec = torch.cat((encoder_output.view(1, self.dim), entity_output.view(1, self.dim)), 0)
		sentence_vec = torch.mean(seq_vec, 0)  # torch.tanh(self.attention(torch.transpose(seq_vec, 1, 2)))  # (1, dim*2, 1)
		...
		inp = torch.mean(torch.cat((sentence_vec.view(1, -1), memory.view(1, -1)), 0), 0)  # (2100-300)
		...
		return inp, prob_noisy


class RLModel(nn.Module):
	def __init__(self, batchsize, dim):
	...
	def forward(self, encoder_output, entity_output, noisy_model, RE_output_tag, relation_target_tensor,
				noisy_sentences_vec, noisy_vec_mean, flag="TRAIN"):
		# sentence_reward_noisy = [0 for i in range(self.batchsize)]
		# noisy_sentences_vec = Variable(torch.FloatTensor(1, self.dim).fill_(0))
		...
		for sentence_id in range(self.batch_size):
			...
			mem, prob_noisy = noisy_model(encoder_output[sentence_id], entity_output[sentence_id],
															noisy_vec_mean, mem, training)
			if prob_noisy[1] > 0.5:
				# print("Reward of noisy: " + str(reward_noisy))
				sentence_vec = encoder_output[sentence_id]  # self.attention(torch.transpose(encoder_output[sentence_id].view(1, -1, self.dim), 1, 2))
				if torch.sum(noisy_sentences_vec):
					noisy_sentences_vec = torch.cat((noisy_sentences_vec, sentence_vec.view(1, -1)), 0)  # np.concatenate
				else:
					noisy_sentences_vec = sentence_vec.view(1, -1)
				# vector of removed/noisy sentences
				noisy_vec_mean = torch.mean(noisy_sentences_vec, 0, True)
			# action_noisy = self.sample(prob_noisy, training)
			# actprob_noisy = prob_noisy[action_noisy]

			# noisy_actions = [action_noisy.item()]
		...
		if flag == "TRAIN":
			...
			loss_RL, relation_reward = Optimize.optimize_each(torch.argmax(RE_output_tag, -1), RE_output_tag,
		...
		if loss_RL != 0.:
			...
			# sentence_reward_noisy[sentence_id] = prob_noisy[1]  # realtion["reward_noisy"] = reward_noisy
		...
		return RE_rewards, loss_RL, noisy_sentences_vec, noisy_vec_mean
