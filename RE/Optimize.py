...
class Optimize(object):
	def __init__(self):
		self.acc_F, self.cnt_F, self.tot_F = 0., 0., 0.

	def calcF1(self, acc, cnt, tot, beta=1.0):
		if cnt == 0 or tot == 0:
			return 0
	...
	def calcREReward(self, top_action, gold_labels):
	...
	def calcREFinalReward(self, RE_actions, labels, top_bias=0.):
		r = 0.
		...
		if cnt != 0:
			r = self.calcF1(acc, cnt, tot, beta=0.9)
		else:
			r = -2
		if cnt > tot:
			r -= 0.5 * (cnt - tot)
	...
	def calcBotGrad(self, bot_action, bot_actprob, bot_reward, bot_final_reward, pretrain=False):
	...
	def calcTopGrad(self, top_action, top_actprob, top_reward, top_final_reward, pretrain=False):
	...
	def optimize(self, RE_actions, top_actprobs, relation_label, entity_loss):  # entity_label, entity_actions, entity_probs,
		...
		top_bias = 0.
		# for i in range(sample_round):
		top_bias += self.calcREFinalReward(RE_actions, relation_label, 0.)
		top_bias /= sample_round
		top_reward = self.calcREReward(RE_actions, relation_label)
		# grads = autograd.Variable(torch.cuda.FloatTensor(1, ).fill_(0))
		...
		# if "RE" in mode:
		grads = self.calcTopGrad(RE_actions, top_actprobs, top_reward, top_bias, pretrain=False)  # +=
		# loss = grads.cpu().data[0]
