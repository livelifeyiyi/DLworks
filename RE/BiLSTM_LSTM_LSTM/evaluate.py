...
def eval_model(encoder, decoder, RE_model, BATCH):
...
if __name__ == "__main__":
	...
	encoder = torch.load(args.modelPath+"model_encoder_epoch20.pkl", map_location=device)
	decoder = torch.load(args.modelPath+"model_decoder_epoch20.pkl", map_location=device)
	RE_model = torch.load(args.modelPath+"relation_model_epoch20.pkl", map_location=device)
	...
	if args.test:
		...
		for b, data in enumerate(mini_batches):
			...
			NER_output, RE_output = eval_model(encoder, decoder, RE_model, args.batchsize)
	...
	if args.test:
		...
		for b, data in enumerate(mini_batches):
			...
			print('*****TEST***** seq-seq NER model: (%d %.2f%%)' % (b, float(b) / batchcnt * 100))
			...
			evaluate.cal_F_score(np.array(action_realtion.cpu()), relation_tags, tags,
								np.array(entity_actions.cpu()), flag="TEST")
