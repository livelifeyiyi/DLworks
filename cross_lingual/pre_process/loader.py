...
def load_sentences(path, to_sort=False):
    ...
    sentence = []
    if 'en' not in path:
        lines = open(path, 'r', encoding='iso-8859-1')
    else:
...
def load_pretrained_emb_weights(id_to_word, pre_emb, word_dim):
    ...
    logger.info('Loading pretrained embeddings from %s...' % pre_emb)
    pretrained = {}
    emb_invalid = 0
    for i, line in enumerate(load_embedding(pre_emb)):
        ...
        if len(line) == word_dim + 1:
            ...
            emb_invalid += 1
    if emb_invalid > 0:
        logger.warning('WARNING: %i invalid lines' % emb_invalid)
    c_found = 0
    c_lower = 0
    c_zeros = 0
    ...
    logger.info('Loaded %i pretrained embeddings.' % len(pretrained))
    logger.info('%i / %i (%.4f%%) words have been initialized with pretrained embeddings.' % (
                  c_found + c_lower + c_zeros, len(id_to_word), 100. * (c_found + c_lower + c_zeros) / len(id_to_word)))
    logger.info('%i found directly, %i after lowercasing, %i after lowercasing + zero.' % (c_found, c_lower, c_zeros))
...
def augment_with_pretrained(all_sentences, ext_emb_path, lower=False):
    ...
    # We add every word in the pretrained file to dico
