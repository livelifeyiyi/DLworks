...
def parse_args(args):
    ...
    params['seq_dis_smooth'] = args.seq_dis_smooth
    ...
    assert not params['related_emb'] or params['target_emb']
    assert not params['target_emb'] or params['word_dim'] > 0
    assert not params['target_emb'] or os.path.isfile(params['target_emb'])
    ...
    model_names = {'signal': args.signal, 'tag_scheme': args.tag_scheme, 'lower': args.lower == 1,
                   ...
                   'target_emb': args.target_emb, 'related_emb': args.related_emb == 1, 'crf': args.crf == 1,
