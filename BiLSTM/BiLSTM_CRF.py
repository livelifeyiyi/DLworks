...
class BILSTM_CRF(object):
	def __init__(self, num_chars, num_classes, num_steps=200, num_epochs=100, embedding_matrix=None, is_training=True,
		...
		self.outputs, _, _ = tf.nn.bidirectional_dynamic_rnn(
			lstm_cell_fw,
			lstm_cell_bw,
			self.inputs_emb,
			dtype=tf.float32,
		...
		# summary
		self.train_summary = tf.contrib.deprecated.scalar_summary("loss", self.loss)
		self.val_summary = tf.contrib.deprecated.scalar_summary("loss", self.loss)
	...
	def train(self, sess, save_file, X_train, y_train, X_val, y_val):
		...
		summary_writer_train = tf.contrib.summary.SummaryWriter('loss_log/train_loss', sess.graph)
		summary_writer_val = tf.contrib.summary.SummaryWriter('loss_log/val_loss', sess.graph)
		...
		for epoch in range(self.num_epochs):
			...
			for iteration in range(num_iterations):
				...
				y_train_weight_batch = 1 + np.array((y_train_batch == label2id['B']) | (y_train_batch == label2id['E']),
													float)
				...
				if iteration % 100 == 0:
					...
					y_val_weight_batch = 1 + np.array((y_val_batch == label2id['B']) | (y_val_batch == label2id['E']),
													  float)
					...
					precision_val, recall_val, f1_val = self.evaluate(X_val_batch, y_val_batch, predicts_val, id2char,
																	  id2label)
					summary_writer_val.add_summary(val_summary, cnt)
					print("iteration: %5d, valid loss: %5d, valid precision: %.5f, valid recall: %.5f, valid f1: %.5f" % (
