...
def load_data():
	...
	:return: Arrays
	"""
	with codecs.open(FLAGS.source_data, 'rb') as f:
	...
	if FLAGS.train:
		...
		for epoch in range(FLAGS.epoch_num):
			...
			if epoch % FLAGS.epochs_per_save == 0:
				save_path = saver.save(sess, "ckpt/model.ckpt", global_step=gstep)
				print("Model saved in path: %s" % save_path)

	else:
		# Load model
		# ckpt = tf.train.get_checkpoint_state('./ckpt/')
		saver.restore(sess, "ckpt/model.ckpt")  # ckpt.model_checkpoint_path
		print('Restore from', "ckpt/model.ckpt")  # ckpt.model_checkpoint_path
